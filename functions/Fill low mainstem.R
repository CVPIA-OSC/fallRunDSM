...
fill.Lmain.func<-function(juvs,flood_hab,juv_hab,territory_size,stoch){
  ...
  if(stoch==0){
    ...
    all.sheds<-pmax(all.sheds-flood_rear,0)
    #In-river rearing
    rear_avail<-juv_hab
    for(i in 3:1){
      i=3
  ...
  list(Lmain.river=river_rear,Lmain.flood =flood_rear,migr=migrants)
}
# 
