...
# 
# flood_hab <- c(100,100,100,100)
# juv_hab <-c(500,500,500,500)
# 
# fill.trib.func(juvs=juvs[1:15,],flood_hab=FP.hab[1:15],juv_hab= RI.hab[1:15],territory_size)

#Assign fish to habitats function, there should be one for each watershed
fill.trib.func<-function(juvs,flood_hab,juv_hab,territory_size,stoch){
  
  if (length(nrow(juvs))> 0) xxx<-nrow(juvs) else xxx <- 1
  ...
  #The rest leave
  migrants<-pmax(juvs-river_rear,0)
