...
inverse logit models whose inputs are monthly stream temperatures. The purpose for these models are to provide a sub monthly metric derived from monthly data, these metrics are needed to calcualte survival probabilities. The script below outlines the parameterization process of these models. 
...
This data was prepped to model the proportion of days in a month with temps > 20 °C and if temperature will go above 25 °C, logistic regression was used to model this phenomenon. 
...
The model for proportion of days where temperature exceeds 20°C in a month is parameterized by fitting a linear
regression model between the log odds of proportion of days in a month greater than 20°C and the monthly mean 
temperature. The logit values resulting from this are converted into proportions using the `boot::inv.logit`.
...
#### Results: temperature exceeding 25°C during month
