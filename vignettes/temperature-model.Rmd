...
Within the `get_rearing_survival()` submodel of the DSM, `aveT20`, `aveT20D`, `maxT25`, and `maxT25D` are defined as
inverse logit models whose inputs are monthly stream temperatures. The purpose for these models are to provide a sub monthly metric derived from monthly data, these metrics are needed to calcualte survival probabilities. The script bellow outlines the parameterization process of these models. 
...
datatable(d, options = list(dom = 't', pageLength = 20))
```

and the following from CDEC: 
...
The table below shows the Central Valley and delta temperatures grouped by month and year and summarized to give monthly mean temperature, proportion of days over 20 degrees, and the count of days per month over 25 degrees. 
...
grouped_temp_measures_delta <- delta_full_temp_data %>%
  group_by(year, month) %>%
  summarise(delta_prop_gt_20 = mean(gt20),
            delta_count_gt_25 = sum(gt25),
            delta_month_mean_temp = mean(TempC))
...
#### Graph logistic model of proportion days temperature exceedes 20°C in a month and monthly mean temp

**Finding `avgT20` and `avgT20D` in DSM models:**
In the DSM models `avgT20` and `avgT20D` is estimated for each month using a logistic model.

**Logistic Model Input:**
The numeric input to our logistic models for CV and Delta temperatures is based on coefficients generated by fitting a linear model on the `odds_ratio` and `month_mean_temp`. We take the sum of our linear model intercept and slope and multiply this by the month mean temperature to find the numeric input for our CV rivers and Delta logistic models:
...
#### Graph temp exceeding 25°C during month and mean monthly temperture 
**Finding `maxT25` and `maxT25D` in DSM models:**
In the DSM models `maxT25` and `maxT25D` is calculated for each month using a logistic model.

**Logistic model input:**
The numeric input to our logistic models for CV and Delta temperature exceeding 25°C is based on coefficients generated by fitting a generalized linear model on the `count_gt_25` and `month_mean_temp`. We take the sum of our linear model intercept and slope and multiply this by the month mean temperature to find the numeric input for our CV rivers and Delta logistic models: 

* **CV Logistic Model**: Numeric Input = ``r glm_summary$coefficients[1, 1]`` + ``r glm_summary$coefficients[2, 1]`` * `month mean temperature`
