...
### Creates logistic models to estimate temperature metrics 
...
#### Central Valley River Temperature Data

The table below shows the Central Valley temperature data grouped by month and year and summarized to give monthly mean temperature, proportion of days over 20 degrees, and the count of days per month over 25 degrees. 
```{r}
...
The table below shows the delta temperature grouped by month and year and summarized to give monthly mean temperature, proportion of days over 20 degrees, and the count of days per month over 25 degrees. 
```{r}
...
full_temp_data <- rbind(jersey_point, dutch_slough, georgiana_slough, potato_slough) %>%
  na.omit() %>%
...
grouped_temp_measures_delta <- full_temp_data %>%
  group_by(year, month) %>%
  summarise(prop_gt_20 = round(mean(gt20), 2),
            count_gt_25 = sum(gt25),
            month_mean_temp = round(mean(TempC), 2))

datatable(grouped_temp_measures_delta %>% rename("Year" = year, 
                                           "Month" = month,
                                           "Number of Measures Exceeding 25 degrees" = count_gt_25,
                                           "Proportion Exceeding 20" = prop_gt_20,
                                           "Monthly Mean Temp" = month_mean_temp),
          options = list(
                    searching = FALSE,
...
Model proportion of days in a month with temps > 20 °C and if temperature will go above 25 °C using logistic regression. 
...
delta_lm_summary <- summary(lm(odds_ratio ~ month_mean_temp, data = delta_grouped_temp_measures_for_logit))
...
delta_plot <- delta_monthly_predictions %>%
  ggplot() +
  geom_line(aes(month_means, prediction), color = "Red") +
  geom_point(data = delta_grouped_temp_measures_for_logit, aes(x = month_mean_temp, y = prop_gt_20)) +
  xlab("Monthly median temperature Delta") +
  ylab("Proportion days temp > 20") + 
...
# Delta model
delta_grouped_temp_measures_for_logit <- grouped_temp_measures_delta %>%
  mutate(count_gt_25 = ifelse(count_gt_25 > 1, 1, count_gt_25))

delta_glm_summary <- summary(glm(count_gt_25 ~ month_mean_temp, data = delta_grouped_temp_measures_for_logit, family='binomial'))
...
delta_plot <- delta_monthly_predictions %>%
  ggplot() +
  geom_line(aes(month_means, prediction), color = "Red") +
  geom_point(data = delta_grouped_temp_measures_for_logit, aes(x = month_mean_temp, y = count_gt_25)) +
